...
const TRANSLATIONS = {
  fr: {
    ...
    themeCustomHintPrimary: "Couleur principale — boutons et éléments actifs.",
    themeCustomHintSecondary: "Couleur secondaire — utilisée pour tes abonnements et tes investissements.",
    themeCustomHintAccent: "Couleur d'accent — utilisée pour tes projets et objectifs d'épargne.",
    ...
    fieldChargeAccountPlaceholder: "Ex : Carte de crédit, Carte de débit",
    fieldDepositAccount: "Déposé dans — optionnel",
    fieldDepositAccountPlaceholder: "Ex : Compte chèque",
    expensesTitle: "Dépense",
    fieldEssential: "Abonnement essentiel",
    fieldEssentialHint: "Téléphone, internet, assurance… On ne te suggérera jamais de l'annuler pour économiser.",
    ...
    fieldExpenseNotePlaceholder: "Ex : Épicerie IGA",
    manageExpensesDesc: "Ajoute tes dépenses du jour, réparties par catégorie",
  ...
  en: {
    ...
    themeCustomHintPrimary: "Main color — buttons and active elements.",
    themeCustomHintSecondary: "Secondary color — used for your subscriptions and investments.",
    themeCustomHintAccent: "Accent color — used for your projects and savings goals.",
    resetToAuto: "Reset",
    ...
    fieldChargeAccountPlaceholder: "E.g. Credit card, Debit card",
    fieldDepositAccount: "Deposited into — optional",
    fieldDepositAccountPlaceholder: "E.g. Chequing account",
    expensesTitle: "Expense",
    fieldEssential: "Essential subscription",
    fieldEssentialHint: "Phone, internet, insurance… We'll never suggest cancelling this to save money.",
    ...
    fieldExpenseNotePlaceholder: "E.g. Grocery run",
    manageExpensesDesc: "Log today's spending, split by category",
...
function pushHistoryPoint(history, total) {
  ...
  if (last && last.date === getToday()) {
    return [...hist.slice(0, -1), { date: getToday(), total }];
  }
  return [...hist, { date: getToday(), total }];
}

// --- Taux de change réels (open.er-api.com — gratuit, sans clé, mis à jour chaque jour) ---

async function fetchExchangeRates(baseCurrency) {
  const res = await fetch(`https://open.er-api.com/v6/latest/${baseCurrency}`);
  if (!res.ok) throw new Error("Requête taux de change échouée");
  const data = await res.json();
  if (data.result !== "success") throw new Error("Réponse API invalide");
  return { base: baseCurrency, rates: data.rates, updatedAt: data.time_last_update_utc || new Date().toISOString() };
}

// Convertit un montant d'une devise vers la devise principale, à partir des taux mis en cache.
// Si aucun taux valide n'est disponible (pas encore chargé, devise inconnue, ou hors-ligne),
// retourne le montant tel quel — mieux vaut une approximation visible que planter l'appli.
function toMainCurrency(amount, fromCurrency, state) {
  const mainCurrency = state.currency;
  if (!fromCurrency || fromCurrency === mainCurrency) return amount;
  const rates = state.exchangeRates;
  if (!rates || rates.base !== mainCurrency || !rates.rates || !rates.rates[fromCurrency]) return amount;
  // rates.rates[X] = combien de X pour 1 unité de mainCurrency → on inverse pour convertir X vers mainCurrency
...
function buildFullCSV(state) {
  ...
  sections.push(csvSection("FACTURES", ["Titre", "Montant", "Devise", "Périodicité", "Jour/Date", "Concerne"],
    state.bills.map((b) => [b.title, b.amount, b.currency || state.currency, b.period, b.startDate || b.day, ownerLabel(b.owner, state)])
  ));
...
function exportAsCSV(state) {
  downloadFile(`serein-export-${getToday()}.csv`, "\uFEFF" + buildFullCSV(state), "text/csv;charset=utf-8;");
}

function exportAsJSON(state) {
  downloadFile(`serein-sauvegarde-${getToday()}.json`, JSON.stringify(state, null, 2), "application/json");
}
...
function printSummary(state) {
  ...
  w.document.write(`
    <html><head><title>Résumé Serein — ${getToday()}</title>
    ...
    <h1>Résumé Serein — ${getToday()}</h1>
    <h2>Dettes</h2><table>${rows("d", state.debts.map((d) => [d.title, money(d.amount - d.paid, d.currency || state.currency) + " restant", ownerLabel(d.owner, state)]))}</table>
    <h2>Projets</h2><table>${rows("p", state.projects.map((p) => [p.title, `${money(p.saved, p.currency || state.currency)} / ${money(p.target, p.currency || state.currency)}`, ownerLabel(p.owner, state)]))}</table>
    ...
    <h2>Investissements</h2><table>${rows("i", state.investments.map((i) => [i.title, i.type, money(i.value, i.currency || state.currency)]))}</table>
...
function debtInstallment(d) {
  ...
  const monthsLeft = monthsBetween(getToday(), d.endDate);
...
function projectInstallment(p) {
  ...
  const monthsLeft = monthsBetween(getToday(), p.endDate);
  ...
  const submit = () => {
    onAdd({
      ...
      account: f.hasAccount
        ? { title: f.accountTitle || "REEE", balance: Number(f.balance) || 0, history: [{ date: getToday(), value: Number(f.balance) || 0 }] }
        : null,
    });
...
const EXPENSE_CATEGORIES = [
  { id: "epicerie", icon: "🛒", type: "besoin", fr: "Épicerie", en: "Groceries" },
  { id: "transport", icon: "🚗", type: "besoin", fr: "Transport", en: "Transport" },
  { id: "sante", icon: "💊", type: "besoin", fr: "Santé", en: "Health" },
  { id: "restaurant", icon: "🍽️", type: "envie", fr: "Restaurant", en: "Dining out" },
  { id: "loisirs", icon: "🎉", type: "envie", fr: "Loisirs", en: "Entertainment" },
...
const PaydaysStep = forwardRef(function PaydaysStep({ theme, state, items, onAdd, onSave, onRemove, editItem, hideList }, ref) {
  ...
  const submit = () => {
    ...
    if (isEdit) {
      ...
      onAdd(item);
      setF({ title: "Salaire", amount: "", frequency: "mensuel", startDate: getToday(), owner: "commun", currency: state.currency, depositAccount: "" });
    }
  };
  useImperativeHandle(ref, () => ({ commit: () => { if (canSubmit) submit(); } }));
  return (
  ...
  return (
    <div>
      <StepShell title={t(lang, "expensesTitle")} hint={t(lang, "expensesHint")} theme={theme} canSubmit={canSubmit} onSubmit={submit} buttonLabel={isEdit ? t(lang, "saveChanges") : undefined} lang={lang}>
            ...
            {EXPENSE_CATEGORIES.map((cat) => (
              <button
                ...
                style={{
                  ...
                  backgroundColor: f.category === cat.id ? theme.soft : "white",
                }}
              >
        ...
        <Field label={t(lang, "fieldExpenseDate")}>
          <input type="date" className={selectCls} style={dateInputStyle} value={f.date} onChange={(e) => setF({ ...f, date: e.target.value })} />
        </Field>
...
function CalendarTab({ theme, state, setState }) {
  ...
  // — converti dans la devise principale via les vrais taux de change quand ils sont disponibles.
  const netWorth = useMemo(() => {
    const byOwner = (arr) => (HAS_OWNERS.includes(state.profileType) && ownerFilter !== "all" ? arr.filter((x) => x.owner === ownerFilter) : arr);
    const investmentsTotal = byOwner(state.investments).reduce((a, i) => a + toMainCurrency(i.value, i.currency || state.currency, state), 0);
    const projectsSaved = byOwner(state.projects).reduce((a, p) => a + toMainCurrency(p.saved, p.currency || state.currency, state), 0);
    const debtsRemaining = byOwner(state.debts).reduce((a, d) => a + toMainCurrency(d.amount - d.paid, d.currency || state.currency, state), 0);
    const assets = investmentsTotal + projectsSaved;
    const hasMixedCurrencies = [...state.investments, ...state.projects, ...state.debts].some((x) => x.currency && x.currency !== state.currency);
    const isConverted = hasMixedCurrencies && state.exchangeRates && state.exchangeRates.base === state.currency;
    return { assets, debts: debtsRemaining, net: assets - debtsRemaining, hasMixedCurrencies, isConverted };
  }, [state, ownerFilter]);
  ...
  return (
    <div className="p-4 pb-24">
      <div className="rounded-xl shadow-sm p-4 mb-4" style={{ backgroundColor: theme.primary }}>
        ...
        {netWorth.hasMixedCurrencies && (
          <p className="text-[10px] text-white opacity-70 mt-2">
            {netWorth.isConverted
              ? `Converti en ${state.currency} — taux mis à jour quotidiennement.`
              : state.exchangeRatesError
              ? `Taux de change indisponibles pour l'instant — montants combinés sans conversion.`
              : `Conversion des devises en cours…`}
          </p>
        )}
      </div>
      ...
      {addMode === "payday" && (
        <ManageListSheet
          ...
          renderRow={(x) => `${x.title} — ${money(x.amount, x.currency || state.currency)} — ${x.frequency}${x.depositAccount ? ` → ${x.depositAccount}` : ""}`}
...
function DebtsTab({ theme, state, setState, onGoToAide }) {
  ...
  const remainingByCurrency = useMemo(() => {
    ...
    filteredDebts.forEach((d) => {
      const cur = d.currency || state.currency;
      totals[cur] = (totals[cur] || 0) + (d.amount - d.paid);
    });
    return Object.entries(totals);
  }, [filteredDebts, state.currency]);

  const cashflow = useMemo(() => {
    const byOwner = (arr) => (HAS_OWNERS.includes(state.profileType) && ownerFilter !== "all" ? arr.filter((x) => x.owner === ownerFilter) : arr);
    const income = byOwner(state.paydays).reduce((a, p) => a + toMainCurrency(monthlyEquivalent(p.amount, p.frequency), p.currency || state.currency, state), 0);
    const billsM = byOwner(state.bills).reduce((a, b) => a + toMainCurrency(monthlyEquivalent(b.amount, b.period), b.currency || state.currency, state), 0);
    const subsM = byOwner(state.subscriptions).reduce((a, s) => a + toMainCurrency(monthlyEquivalent(s.amount, s.period), s.currency || state.currency, state), 0);
    const debtsM = byOwner(state.debts).reduce((a, d) => a + toMainCurrency(monthlyEquivalent(debtInstallment(d), d.frequency), d.currency || state.currency, state), 0);
    const projectsM = byOwner(state.projects).reduce((a, p) => a + toMainCurrency(monthlyEquivalent(projectInstallment(p), p.frequency), p.currency || state.currency, state), 0);
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            {t(lang, "remainingToRepay")} : {remainingByCurrency.length === 0
              ? money(0, state.currency)
              : remainingByCurrency.map(([cur, amt]) => money(amt, cur)).join(" + ")}
          </p>
        </div>
        <div className="flex gap-2">
...
function InvestmentsTab({ theme, state, setState }) {
  ...
  const addInvestmentItem = (item) => {
    setState((s) => ({ ...s, investments: [...s.investments, { id: uid(), ...item }] }));
  ...
  const updateValue = (id) => {
    ...
    setState((s) => ({
      ...
      investments: s.investments.map((inv) =>
        inv.id === id
          ? { ...inv, value: val, history: [...inv.history, { date: getToday(), value: val }] }
          : inv
      ),
    }));
    ...
    setState((s) => ({
      ...
      children: s.children.map((c) =>
        c.id === newChildAccount.childId
          ? { ...c, account: { title: newChildAccount.title || "REEE", balance, history: [{ date: getToday(), value: balance }] } }
          : c
      ),
    }));
    ...
    setState((s) => ({
      ...
      children: s.children.map((c) =>
        c.id === childId && c.account
          ? { ...c, account: { ...c.account, balance: val, history: [...c.account.history, { date: getToday(), value: val }] } }
          : c
      ),
    }));
  ...
  const totalsByCurrency = useMemo(() => {
    ...
    filteredInvestments.forEach((i) => {
      const cur = i.currency || state.currency;
      totals[cur] = (totals[cur] || 0) + i.value;
    });
    return Object.entries(totals); // [[devise, montant], ...]
  }, [filteredInvestments, state.currency]);
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">
            {totalsByCurrency.length === 0
              ? `Total : ${money(0, state.currency)}`
              : totalsByCurrency.map(([cur, amt]) => money(amt, cur)).join(" + ")}
          </p>
        </div>
...
function SettingsPanel({ theme, state, setState, onClose, onReset }) {
  ...
  const addChildItem = (item) => {
    setState((s) => ({ ...s, children: [...s.children, { id: uid(), ...item }] }));
    setShowAddChild(false);
  };
  const removeChild = (id) => setState((s) => ({ ...s, children: s.children.filter((c) => c.id !== id) }));
  const addPaydayItem = (item) => {
    setState((s) => ({ ...s, paydays: [...s.paydays, { id: uid(), ...item }] }));
  ...
  const addAidantItem = (item) => {
    setState((s) => ({ ...s, people: [...s.people, { id: uid(), ...item }] }));
...
function AideTab({ theme, state }) {
  ...
  // Dépenses journalisées ce mois-ci (converties dans la devise principale), réparties par catégorie
  const now = new Date();
  ...
  const needsMonthly =
    state.bills.reduce((a, b) => a + toMainCurrency(monthlyEquivalent(b.amount, b.period), b.currency || state.currency, state), 0) +
    state.debts.reduce((a, d) => a + toMainCurrency(monthlyEquivalent(debtInstallment(d), d.frequency), d.currency || state.currency, state), 0) +
    expensesBesoin;
  const wantsMonthly = state.subscriptions.reduce((a, s) => a + toMainCurrency(monthlyEquivalent(s.amount, s.period), s.currency || state.currency, state), 0) + expensesEnvie;
  const savingsMonthly = state.projects.reduce((a, p) => a + toMainCurrency(monthlyEquivalent(projectInstallment(p), p.frequency), p.currency || state.currency, state), 0);
  ...
  const nonEssentialSubs = sortedSubs.filter((s) => !s.essential);
  const topSub = nonEssentialSubs[0];
  ...
  const envelopes = [
    ...state.debts.map((d) => ({ id: d.id, title: d.title, pct: d.amount ? (d.paid / d.amount) * 100 : 0, sub: `${money(d.amount - d.paid, d.currency || state.currency)} ${t(lang, "remainingWord")}`, color: theme.primary })),
    ...state.projects.map((p) => ({ id: p.id, title: p.title, pct: p.target ? (p.saved / p.target) * 100 : 0, sub: `${money(p.saved, p.currency || state.currency)} / ${money(p.target, p.currency || state.currency)}`, color: theme.accent })),
  ];
  ...
  return (
    <div className="p-4 pb-24">
      ...
      {expensesByCategory.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
          <h3 className="text-sm font-medium mb-3 flex items-center gap-2"><ReceiptText size={15} /> {t(lang, "thisMonthSpending")}</h3>
          <ul className="space-y-2">
            {[...expensesByCategory].sort((a, b) => b.total - a.total).map((cat) => (
              <li key={cat.id}>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-slate-600">{cat.icon} {lang === "en" ? cat.en : cat.fr}</span>
                  <span className="text-slate-400">{money(cat.total, state.currency)}</span>
                </div>
                <div className="w-full h-2 rounded-full bg-slate-100 overflow-hidden">
                  <div
                    className="h-full rounded-full"
                    style={{
                      width: `${Math.min(100, (cat.total / expensesByCategory.reduce((a, c) => a + c.total, 0)) * 100)}%`,
                      backgroundColor: cat.type === "besoin" ? theme.primary : theme.accent,
                    }}
                  />
                </div>
              </li>
          ...
          {expensesByCategory[0] && (
            <p className="text-xs mt-3 p-2 rounded-lg" style={{ backgroundColor: theme.soft, color: theme.text }}>
              {t(lang, "spentOnCategory")
                .replace("{amount}", money([...expensesByCategory].sort((a, b) => b.total - a.total)[0].total, state.currency))
                .replace("{category}", lang === "en" ? [...expensesByCategory].sort((a, b) => b.total - a.total)[0].en : [...expensesByCategory].sort((a, b) => b.total - a.total)[0].fr)}
            </p>
          )}
        </div>
      )}
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
          ...
          {topSub ? (
            <p className="text-xs mb-3 p-2 rounded-lg" style={{ backgroundColor: theme.soft, color: theme.text }}>
              {t(lang, "aideCancelHintNonEssential").replace("{title}", topSub.title).replace("{amount}", money(monthlyEquivalent(topSub.amount, topSub.period), state.currency))}
            </p>
          ) : (
            <p className="text-xs mb-3 p-2 rounded-lg" style={{ backgroundColor: theme.soft, color: theme.text }}>
              {t(lang, "aideAllEssential")}
            </p>
          )}
          <ul className="space-y-1.5">
            {sortedSubs.map((s) => (
              <li key={s.id} className="flex justify-between text-sm">
                <span className="text-slate-600 flex items-center gap-1.5">
                  {s.title}
                  {s.essential && (
                    <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full" style={{ backgroundColor: theme.soft, color: theme.text }}>
                      {t(lang, "fieldEssential")}
                    </span>
                  )}
                </span>
...
export default function App() {
  ...
  // continue avec les derniers taux connus (ou aucun) sans planter l'appli.
  useEffect(() => {
    if (!state) return;
    const cached = state.exchangeRates;
    const isStale = !cached || cached.base !== state.currency || (Date.now() - new Date(cached.fetchedAt || 0).getTime()) > 20 * 60 * 60 * 1000;
    if (!isStale) return;
    let cancelled = false;
    fetchExchangeRates(state.currency)
      .then((result) => {
        if (cancelled) return;
        setState((s) => ({ ...s, exchangeRates: { ...result, fetchedAt: new Date().toISOString() }, exchangeRatesError: false }));
      })
      .catch(() => {
        if (cancelled) return;
        setState((s) => ({ ...s, exchangeRatesError: true }));
      });
    return () => { cancelled = true; };
